<%@ page import="java.util.List, java.util.ArrayList, java.time.LocalDateTime, java.time.format.DateTimeFormatter, chen_yi_an.*, chen_kai_bo.Project, chen_zi_hao.Order" %>
<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
<%
    User loginUser = (User) session.getAttribute("user");
    
    int projectId = request.getAttribute("projectId") != null ? (int) request.getAttribute("projectId") : 0;
    ...
    } else if (project != null && "open".equals(project.getStatus())) { progressPct = 10; progressLabel = "📋 招募中"; }
    else if (project != null && "cancelled".equals(project.getStatus())) { progressPct = 0; progressLabel = "❌ 已取消"; }
%>
<!DOCTYPE html>
<html lang="zh-CN">
<head>
    ...
    <title>交付与沟通 - <%= projectTitle != null ? projectTitle : "" %> - Freelite</title>
...
<body>
    ...
    <div class="container mt-4" style="max-width: 780px;">
        ...
        <% } %>
        <% if (loginUser == null) { %>
            <div class="alert alert-info">请先 <a href="${pageContext.request.contextPath}/login">登录</a> 以参与项目沟通</div>
        <% } %>
        ...
        <p class="text-muted mb-3"><%= projectTitle != null ? projectTitle : "" %></p>

        <%-- 进度条 --%>
        <% if (progressPct > 0 || (project != null && "open".equals(project.getStatus()))) { %>
            ...
            <div class="d-flex justify-content-between align-items-center">
                ...
                <span style="font-size: 0.8rem; color: #6b7280;">
                    <% if (order != null) { %>
                        ...
                        <% } %>
                    <% } else if (project != null && "open".equals(project.getStatus())) { %>等待竞标中<% } %>
                </span>
            </div>
        </div>
        <% } %>
        ...
        <div class="tab-content">
                ...
                <div class="card p-3 mb-3" style="min-height: 300px; max-height: 450px; overflow-y: auto;">
                    <% if (messages == null || messages.isEmpty()) { %>
                        <p class="text-muted text-center my-5">
                            <% if (loginUser == null) { %>请先登录以查看消息<% } else { %>暂无消息，发送第一条消息吧<% } %>
                        </p>
                    <% } else { %>
                        <% for (ProjectMessage msg : messages) {
                            boolean isSelf = loginUser != null && msg.getSenderId() == loginUser.getId();
                        %>
            ...
            <div class="tab-pane fade" id="delivery" role="tabpanel">
                ...
                <%-- 上传交付物（仅自由职业者，已登录） --%>
                <% if (loginUser != null && isFreelancer && order != null && "in_progress".equals(order.getStatus())) { %>
                    <div class="card p-3">
